...
impl Config {
    pub fn load_path(path: impl AsRef<Path>) -> Result<Self> {
        ...
        toml::from_str(&raw).map_err(|e| DaemonError::Invalid(format!("config: {e}")))
    }
...
fn env(key: &str) -> Result<String> {
    std::env::var(key).map_err(|_| DaemonError::Invalid(format!("missing token env {key}")))
}

pub fn config_path_from_args() -> PathBuf {
    let mut args = std::env::args().skip(1);
    match args.next().as_deref() {
        Some("--config") => PathBuf::from(args.next().unwrap_or_else(|| "daemon.toml".into())),
        Some(p) if !p.starts_with('-') => PathBuf::from(p),
        _ => PathBuf::from("daemon.toml"),
    }
}
